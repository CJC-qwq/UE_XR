...
## 2026-05-18 17:14:58 Git 历史清理并切换为提交级时间戳 Tag 规则

### 任务

* 按你的要求移除中间那条不想保留的 `a30f283` 提交路径
* 将仓库版本规则固定为“每个保留提交都必须有自己的时间戳 tag”
* 让 GitHub 上后续查版本时只保留你真正想看的提交节点

### 处理

* 将本地主线回退到你确认过的功能版本提交：
  * `dc83ee9`
* 重新补一条规范化提交，而不是继续保留之前那条单独的里程碑 tag 记录提交
* 更新项目规约：
  * `Agents/AGENTS.md`
* 新增 Git 版本规则：
  * 每个保留提交都必须有自己的 annotated tag
  * tag 名统一使用时间戳格式：
    * `vYYYY.MM.DD-HHMMSS-short-slug`
  * 如果某个提交被从保留历史里移除，则也要删除只指向该提交的 tag
* 准备将清理后的主线和新的提交级 tag 一起推回 GitHub
